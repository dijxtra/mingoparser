...
class SQLTest(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        baza = DatabaseConnection('db.test-1.sqlite3')
        baza.kreiraj_tablice()

        citac = CitacVrijednostiOffline('inputs/')
        baza.pisi_nove_vrijednosti(citac, datum = '2015-02-23 11:23:45')

    def setUp(self):
        self.baza = DatabaseConnection('db.test-1.sqlite3')
        ...
        for vlasnik in sortirani_vlasnici:
            if vlasnik.nudi_gorivo(vrsta_goriva):
                if vlasnik.broj_postaja(vrsta_goriva) >= limit:
                    self.assertEqual(vlasnik.ime(), ocekivani_rezultati[i][0])
                    i += 1

    def test_cijene_s_brojem_postaja(self):
        vrsta_goriva = 2
        limit = 4
        ...
        for vlasnik in sortirani_vlasnici:
            if vlasnik.nudi_gorivo(vrsta_goriva):
                if vlasnik.broj_postaja(vrsta_goriva) >= limit:
                    self.assertEqual(vlasnik.cijene_sa_brojem_postaja(vrsta_goriva), ocekivani_rezultati[i][1])
                    i += 1
    ...
    def test_broj_postaja(self):
        ...
        for vlasnik in sortirani_vlasnici:
            if vlasnik.nudi_gorivo(vrsta_goriva):
                if vlasnik.broj_postaja(vrsta_goriva) >= limit:
                    self.assertEqual(vlasnik.broj_postaja(vrsta_goriva), ocekivani_rezultati[i][2])
                    i += 1
    ...
    def test_indeksi(self):
        ...
        for vlasnik in sortirani_vlasnici:
            if vlasnik.nudi_gorivo(vrsta_goriva):
                if vlasnik.broj_postaja(vrsta_goriva) >= limit:
                    self.assertEqual(round(vlasnik.indeks(vrsta_goriva), 2), ocekivani_rezultati[i][3])
                    i += 1
